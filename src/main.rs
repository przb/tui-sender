...
fn server<A>(addr: A, ready: Sender<()>) -> Result<()>
where
    A: ToSocketAddrs,
{
...
fn client<A>(addr: A, read: Receiver<()>) -> Result<()>
where
    A: ToSocketAddrs,
{
    read.recv().unwrap();
...
fn main() -> Result<()> {
    ...
    let client_jh = std::thread::spawn(|| client(client_addr, ready_rx));
    let server_jh = std::thread::spawn(|| server(server_addr, ready_tx));
